<template>
  <div v-if="currentUser" class="sidebar">
    <div class="sidebar-wrapper">
      <div class="logo">
        <router-link class="no-link" :to="{name: 'home'}">
          <div class="logo-img">
            <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
	 viewBox="0 0 276 276" style="enable-background:new 0 0 276 276;" xml:space="preserve">
              <g>
                <g>
                  <path d="M247.064,75.156c-19.134-13.736-40.911-13.342-40.911-13.342
                    ...
                    />
                  <polygon points="138.003,214.186 138.003,214.185 138.002,214.186"/>
                </g>
              </g>
            </svg>
            Time Manager</div>
      ...
      <ul class="nav navbar-nav">
        <li class="nav-item" :class="{active : ['home'].indexOf($route.name) > -1}">
          <router-link class="nav-link" :to="{name: 'home'}">
            <i class="fa fa-tachometer" aria-hidden="true"></i> Dashboard
          </router-link>
        </li>
        <li class="nav-item" :class="{active : ['clock'].indexOf($route.name) > -1}">
          <router-link class="nav-link" :to="{name: 'clock'}">
            <i class="fa fa-clock-o" aria-hidden="true"></i> Clock
          </router-link>
        </li>
        <li class="nav-item" :class="{active : ['chart'].indexOf($route.name) > -1}">
          <router-link class="nav-link" :to="{name: 'chart'}">
            <i class="fa fa-pie-chart" aria-hidden="true"></i> Chart Manager
          </router-link>
        </li>
        <li class="nav-item" :class="{active : ['profile'].indexOf($route.name) > -1}">
          <router-link class="nav-link" :to="{name: 'profile'}">
            <i class="fa fa-user" aria-hidden="true"></i> Profile
          </router-link>
        </li>
        <li v-if="currentUser && currentUser.role_id >= 2" class="nav-item">
          <router-link class="no-link" :to="{name: 'manager'}">
            <i class="fa fa-cogs" aria-hidden="true"></i> Management
          </router-link>
        </li>
        <li v-if="currentUser && currentUser.role_id >= 3" class="nav-item">
          <router-link class="no-link" :to="{name: 'admin'}">
            <i class="fa fa-user-secret" aria-hidden="true"></i> Admin
          </router-link>
        </li>
        <li v-if="currentUser && currentUser.role_id >= 3" class="nav-item">
          <router-link class="no-link" :to="{name: 'admin/globalgraph'}">
            <i class="fa fa-line-chart" aria-hidden="true"></i> Global graphics
          </router-link>
        </li>
      </ul>
    </div>
